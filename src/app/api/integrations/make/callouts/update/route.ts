...
export async function POST(req: NextRequest) {
  ...
  try {
    const updatedCount = await prisma.$executeRaw`
      ...
      WHERE "id" = ${id};
    `;
    if (updatedCount === 0) return NextResponse.json({ error: "Callout not found", id }, { status: 404 });
    ...
    return NextResponse.json({ error: "DB error (update)", detail: String(e?.message ?? e) }, { status: 500 });
  }
}
